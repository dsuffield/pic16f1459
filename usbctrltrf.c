/************************************************************************************\
  ...
  usbctrltrf.c - PIC16F1489 USB driver
...
__near unsigned char *pSrc;                       // Data source pointer
__near unsigned char *pDst;                       // Data destination pointer
...
{   
    if(USTAT == EP00_OUT)
    {
        if(bdt_data.ep_bd_pairs[0].ep_bd_out.Stat.PID == SETUP_TOKEN)           // EP0 SETUP
 ...
 *
 * PreCondition:    bdt_data.SetupPkt buffer is loaded with valid USB Setup Data
 *
 * Input:           None
 *
 * Output:          None
...
void USBCtrlTrfSetupHandler(void)
{
    ...
    if(ctrl_trf_session_owner == MUID_NULL)
        rt_check_request();            // No body claimed this Setup Request, so call our class handler.
...
void USBCtrlTrfOutHandler(void)
{
    if(ctrl_trf_state == CTRL_TRF_RX)
    {
         ...
         */
        if(bdt_data.ep_bd_pairs[0].ep_bd_out.Stat.DTS == 0)
            bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE|_DAT1|_DTSEN;
        else
            bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE|_DAT0|_DTSEN;
    }
    else    // CTRL_TRF_TX
...
void USBCtrlTrfInHandler(void)
{
    ...
    if(ctrl_trf_state == CTRL_TRF_TX)
    {
        ...
        if(bdt_data.ep_bd_pairs[0].ep_bd_in.Stat.DTS == 0)
            bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _USIE|_DAT1|_DTSEN;
        else
            bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _USIE|_DAT0|_DTSEN;
    }
    else // CTRL_TRF_RX
        USBPrepareForNextSetupTrf();
...
{    
     ...
     */
    bdt_data.ep_bd_pairs[0].ep_bd_in.Stat.BC9 = 0;
    bdt_data.ep_bd_pairs[0].ep_bd_in.Stat.BC8 = 0;
    bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte |= MSB(byte_to_send);
    bdt_data.ep_bd_pairs[0].ep_bd_in.Cnt = LSB(byte_to_send);
    ...
    pDst = (__near unsigned char *)&bdt_data.CtrlTrfData;        // Set destination pointer

    if(usb_stat.ctrl_trf_mem == _ROM)       // Determine type of memory source
    {
        while(byte_to_send._word)
        {
            *pDst = *(const unsigned char *)pSrc;
            pDst++;
            pSrc++;
            byte_to_send._word--;
...
void USBCtrlTrfRxService(void)
{
    ...
    MSB(byte_to_read) = 0x03 & bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte;    // Filter out last 2 bits
    LSB(byte_to_read) = bdt_data.ep_bd_pairs[0].ep_bd_out.Cnt;
    ...
    pSrc = (__near unsigned char *)&bdt_data.CtrlTrfData;
...
void USBCtrlEPServiceComplete(void)
{
    ...
    if(ctrl_trf_session_owner == MUID_NULL)
    {
         ...
         */
        bdt_data.ep_bd_pairs[0].ep_bd_out.Cnt = EP0_BUFF_SIZE;
        bdt_data.ep_bd_pairs[0].ep_bd_out.ADR = (__near byte *)&bdt_data.SetupPkt;
        
        bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE|_BSTALL;
        bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _USIE|_BSTALL;
    }
    else    // A module has claimed ownership of the control transfer session.
    {
        if(bdt_data.SetupPkt.DataDir == DEV_TO_HOST)
        {
            if(bdt_data.SetupPkt.wLength < wCount._word)
                wCount._word = bdt_data.SetupPkt.wLength;
            USBCtrlTrfTxService();
             ...
             * should be pointed to bdt_data.SetupPkt.
             */
            bdt_data.ep_bd_pairs[0].ep_bd_out.Cnt = EP0_BUFF_SIZE;
            bdt_data.ep_bd_pairs[0].ep_bd_out.ADR = (__near byte *)&bdt_data.SetupPkt;            
            bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE;           // Note: DTSEN is 0!
    
            /*
             * 2. Prepare IN EP to transfer data, Cnt should have
             *    been initialized by responsible request owner.
             */
            bdt_data.ep_bd_pairs[0].ep_bd_in.ADR = (__near byte *)&bdt_data.CtrlTrfData;
            bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _USIE|_DAT1|_DTSEN;
        }
        else    // (bdt_data.SetupPkt.DataDir == HOST_TO_DEV)
        {
             ...
             */
            bdt_data.ep_bd_pairs[0].ep_bd_in.Cnt = 0;
            bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _USIE|_DAT1|_DTSEN;
             ...
             */
            bdt_data.ep_bd_pairs[0].ep_bd_out.Cnt = EP0_BUFF_SIZE;
            bdt_data.ep_bd_pairs[0].ep_bd_out.ADR = (__near byte *)&bdt_data.CtrlTrfData;
            bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE|_DAT1|_DTSEN;
        }//end if(bdt_data.SetupPkt.DataDir == DEV_TO_HOST)
...
void USBPrepareForNextSetupTrf(void)
{
    ...
    bdt_data.ep_bd_pairs[0].ep_bd_out.Cnt = EP0_BUFF_SIZE;              // Defined in usbcfg.h
    bdt_data.ep_bd_pairs[0].ep_bd_out.ADR = (__near byte *)&bdt_data.SetupPkt;
    bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _DAT0|_DTSEN;  // EP0 buff dsc init, see usbmmap.h
    bdt_data.ep_bd_pairs[0].ep_bd_out.Stat._byte = _USIE;      // Set last
    bdt_data.ep_bd_pairs[0].ep_bd_in.Stat._byte = _UCPU;               // EP0 IN buffer initialization
