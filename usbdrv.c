/************************************************************************************\
  ...
  usbdrv.c - PIC16F1489 USB driver
...
__at(0x020) volatile BDTDATA bdt_data;                      //Buffer Descriptor data
...
void USBCheckBusStatus(void)
{
     ...
     * usb_bus_sense is an i/o pin defined in rt-usb.h
     *************************************************************************/
    #define USB_BUS_ATTACHED    1
    #define USB_BUS_DETACHED    0

    if(USB_BUS_SENSE_PIN == USB_BUS_ATTACHED)       // Is USB bus attached?
    {
...
{   
    ...
    if(UIRbits.IDLEIF && UIEbits.IDLEIE)
       USBSuspend();        /* 3ms idle condition put CPU into sleep mode */
    ...
    if(UIRbits.TRNIF && UIEbits.TRNIE)
    {
       ...
       /* Process class specific io here while USTAT is valid. */
//       rt_class_service();
//        if (USTAT == EP01_OUT) 
//           tmp_cnt++;
        UIRbits.TRNIF = 0;
